...
public:
    ...
    bool healthy() const { return _worker.healthy(); }
    bool inputFrame(const Frame::Ptr& frame) override {
        if (!healthy()) return _worker.submit([](){});
        ...
        return _worker.submit([recorder = _recorder, cached]() { recorder->inputFrame(cached); }, cached->size());
    }
